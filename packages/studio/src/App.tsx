...
export function App() {
  const [deck, setDeck] = useState<DeckJson>(EXAMPLE_DECK);
  ...
  const html = useMemo(() => {
    try {
      ...
      return `<pre style="color:#f87171;font-family:monospace;padding:24px">${String(err)}</pre>`;
    }
  }, [deck]);
  ...
  return (
    <div className="app">
      <Toolbar deck={deck} onChange={setDeck} onLoadExample={loadExample} />
